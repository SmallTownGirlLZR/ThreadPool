...
# Skill is the parsed representation of one skill file.
@dataclass
class Skill:
    name: str
    # A short human-readable summary of what the skill does. The TUI uses this to show the slash-command autocomplete list.
    description: str
    system_prompt_template: str
    # When the skill is invoked, only these tools are exposed to the LLM.
...
def _parse_skill_file(path: Path) -> Skill:
    ...
    name = path.stem    #  path.stem returns the filename without its final extension.
    description = ""
    ...
    if m:
        ...
        while i < len(lines):
            ...
            if stripped.startswith("name:"):
                ...
                if val in (">", "|"):
                    """
                    > : fold all lines into a single line, replacing newlines with spaces
                    | : keep all lines as-is, preserving newlines
                    """
                    fold = val == ">"
                    parts: list[str] = []
                    i += 1
