...
class SpawnAgentTool(BaseTool):
    ...
    async def invoke(self, params: dict[str, object]) -> ToolResult:
        ...
        # load from a toml file if subagent_type is specified; otherwise, use default profile
        if p.subagent_type:
    ...
    def _build_child_registry(
        ...
        # allow subagent to spawn nested subagents only if depth < 1 (i.e., root agent can spawn, but subagents cannot spawn further)
        if self._depth < 1:
