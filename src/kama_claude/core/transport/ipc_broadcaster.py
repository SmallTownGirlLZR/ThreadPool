...
class _Subscription:
    sub_id: str
    writer: asyncio.StreamWriter    # One client can have multiple subscriptions, but each subscription is associated with one writer
    topics: list[str]       # Filter events by topic, such as run.* or tool.*
    # A client can launch multiple agent runs, and the core daemon is designed to run them asynchronously.
    scope: str      # Filter events by scope, such as global or run:<run_id>
...
class IpcEventBroadcaster:
    ...
    # Push an event to matching subscribers and defer cleanup of dead connections.
