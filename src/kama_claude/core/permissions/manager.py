...
"""
The policy file is not scoped to one session. 
It is persistent and typically applies across all sessions handled by the same core daemon, 
and across restarts after being loaded again.
"""
class PermissionManager:
    def __init__(
        ...
        *,
        ...
        # (session_id, tool_name) → "allow" | "deny"（session 内存，重启丢失）, It is populated when the user chooses always_allow or always_deny
    ...
    async def check_and_wait(
        ...
        tool_use_id: str,
        tool_name: str,
        params: dict[str, Any],     # It comes from the LLM’s tool-call input:
        session_id: str,
        ...
        # A match means the command is immediately denied:
        ...
        # If it matches, the command is not automatically denied. Instead, it is forced to require user approval:
