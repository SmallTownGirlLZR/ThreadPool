...
class SessionManager:
    ...
    async def send_message(self, sid: str, content: str, *, run_id: str | None = None) -> str:
        ...
        async with lock:
            ...
            # append message to thread.jsonl and publish event
            ...
            # the metedata of the session is updated (run_id, updated_at), so we need to write it back to meta.json
            self._store.write_meta(session)

            # Skill 解析：检测 "/" 前缀，展开为系统提示覆盖和工具白名单
            # /review src/kama_claude/core
            """
            content[1:]        # "review src/kama_claude/core"
            parts              # ["review", "src/kama_claude/core"]
            skill_name         # "review"
            arguments          # "src/kama_claude/core"
            """
            goal = content
            ...
            if content.startswith("/"):
                ...
                skill = self._skill_loader.resolve(skill_name)      # Return a Skill object if the skill is found, otherwise return None.
                if skill is not None:
                    # goal : system prompt with arguments --- user prompt
                    goal = self._skill_loader.render_prompt(skill, arguments)
                    # original skill template, still containing "$ARGUMENTS"    system prompt
    ...
    async def compact(self, sid: str, focus: str = "") -> Any:
        ...
        async with lock:
            ...
            messages = self._store.read_messages(sid)       # message from thread.jsonl
