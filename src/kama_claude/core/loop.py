        ...
        while not context.is_done():
            ...
            try:
                response = await self._provider.chat(
                    ...
                    tool_schemas=self._registry.tool_schemas(), # bind tools
                    bus=self._bus,
            ...
            if (
                not context.is_done()       # Only compact an active run
                and response.stop_reason == "tool_use"
                and self._compactor is not None
                and self._compact_threshold > 0     # A threshold of 0 means never compact automatically.
                and response.usage is not None      # Context usage information must exist
